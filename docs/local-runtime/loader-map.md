...
| `data:` | `data:image/gif;base64,...` | Existing data URL loader decodes self-contained bytes | In package mode, permit fallthrough to Servo's existing `data:` path; do not rewrite as `asset:` or give package authority | destination and MIME from data URL | Yes |
